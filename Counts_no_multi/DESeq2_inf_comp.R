...
dir.create("DESeq2_output", showWarnings = F)
setwd("DESeq2_output")
...
ddsHTSeq$phase2_cat=relevel(ddsHTSeq$inf_status, ref = reference_in)
levels(ddsHTSeq$inf_status)
...
res1_lrt = results(ddskeep_lrt,contrast=c("inf_status","infected","control"), alpha = 0.05)
write.csv(res1_lrt[order(res1_lrt["padj"]),], file = "DESeq2_LRT_OutputTables/DESeq2_inf_vs_ctrl.csv")
...
fiss <- plotCounts(dds_lrt, gene=rownames(top), 
                   intgroup = "inf_status", returnData = T)
most_sig = ggplot(fiss,
                  aes(x = inf_status, y = count, color = inf_status, group = 1)) + 
...
gene_to_test = "CD8A"
gene = plotCounts(dds_lrt,gene_to_test, returnData = T, normalized = T, intgroup = "inf_status")
...
#PCA Plots
library(ggplot2)
PCA=plotPCA(vsd, intgroup = c("inf_status"), ntop = 100000, returnData = TRUE)
percentVar <- round(100 * attr(PCA, "percentVar"))
PCA2 = ggplot(data = PCA, aes(PC1, PC2, color = inf_status)) +
  geom_point(size=3) +
...
volcano_lrt = EnhancedVolcano(res1_lrt,
                ...
                selectLab = rownames(res1_lrt)[res1_lrt$padj <= 0.05], #label 20 most signifcant ASVs
                drawConnectors = TRUE, 
...
ColourStrategy = metadata$inf_status
