#!/usr/bin/env Rscript
setwd("/Users/ap14958/OneDrive - University of Bristol/Genomics Facility Bioinformatics/Project #12 Swine Flu - Amy Thomas/Counts_multi/")
...
countData <- as.matrix(read.table(gene_counts_in, header=TRUE,
                                  row.names = 1, sep = "\t",
                                  as.is=TRUE))
...
levels(ddsHTSeq$phase2_cat)
levels(ddsHTSeq$replicate)
...
# 
# # ddsHTSeq$phase2_cat
# levels =levels(ddsHTSeq$phase2_cat)
...
# 
# res1 = results(ddskeep,contrast=c("phase2_cat","P0","P1"), alpha = 0.05)
# res1
# write.csv(res1[order(res1["padj"]),], file = "DESeq2_Pairwise_OutputTables/DESeq2_P0_vs_P1.csv")
# 
# res2 = results(ddskeep,contrast=c("phase2_cat","P0","P2"), alpha = 0.05)
# res2
# res2[order(res2["padj"]),]
# write.csv(res2[order(res2["padj"]),], file = "DESeq2_Pairwise_OutputTables/DESeq2_P0_vs_P2.csv")
# 
# res3 = results(ddskeep,contrast=c("phase2_cat","P0","P3"), alpha = 0.05)
# res3
# res3[order(res3["padj"]),]
# write.csv(res3[order(res3["padj"]),], file = "DESeq2_Pairwise_OutputTables/DESeq2_P0_vs_P3.csv")
# 
# res4 = results(ddskeep,contrast=c("phase2_cat","P0","P4"), alpha = 0.05)
# res4
# res4[order(res4["padj"]),]
# write.csv(res4[order(res4["padj"]),], file = "DESeq2_Pairwise_OutputTables/DESeq2_P0_vs_P4.csv")
# 
# res5 = results(ddskeep,contrast=c("phase2_cat","P0","P5"), alpha = 0.05)
# res5
# res5[order(res5["padj"]),]
# write.csv(res5[order(res5["padj"]),], file = "DESeq2_Pairwise_OutputTables/DESeq2_P0_vs_P5.csv")
# 
# # send normalized counts to tab delimited file for GSEA, etc.
# 
# outputPrefix = "All"
# write.csv(as.data.frame(counts(dds, normalized=T)), file = "All_normalized_counts_DESeq2.csv")

####################################################################
########################### DGE - LRT ##############################
####################################################################

# Set to Likelihood Ratio Test 
# Genes with small P values are those which from one or more replicate showed a Phase-Specific change.

dds_lrt <- DESeq(ddsHTSeq, test = "LRT", reduced = ~replicate)
keep_lrt <- rowSums(counts(dds_lrt)) >= 1
ddskeep_lrt = dds_lrt[keep_lrt,]
results_lrt = results(ddskeep_lrt)
results_lrt[order(results_lrt["padj"]),]

dir.create("DESeq2_LRT_OutputTables", showWarnings = FALSE)

res1_lrt = results(ddskeep_lrt,contrast=c("phase2_cat","P0","P1"), alpha = 0.05)
write.csv(res1_lrt[order(res1_lrt["padj"]),], file = "DESeq2_LRT_OutputTables/DESeq2_P0_vs_P1.csv")

res2_lrt = results(ddskeep_lrt,contrast=c("phase2_cat","P0","P2"), alpha = 0.05)
write.csv(res2_lrt[order(res2_lrt["padj"]),], file = "DESeq2_LRT_OutputTables/DESeq2_P0_vs_P2.csv")

res3_lrt = results(ddskeep_lrt,contrast=c("phase2_cat","P0","P3"), alpha = 0.05)
write.csv(res3_lrt[order(res3_lrt["padj"]),], file = "DESeq2_LRT_OutputTables/DESeq2_P0_vs_P3.csv")

res4_lrt = results(ddskeep_lrt,contrast=c("phase2_cat","P0","P4"), alpha = 0.05)
write.csv(res4_lrt[order(res4_lrt["padj"]),], file = "DESeq2_LRT_OutputTables/DESeq2_P0_vs_P4.csv")

res5_lrt = results(ddskeep_lrt,contrast=c("phase2_cat","P0","P5"), alpha = 0.05)
write.csv(res5_lrt[order(res5_lrt["padj"]),], file = "DESeq2_LRT_OutputTables/DESeq2_P0_vs_P5.csv")
...
gene_to_test = "CD8A"
gene = plotCounts(dds_lrt,gene_to_test, returnData = T, normalized = T, intgroup = "phase2_cat")
...
# #Loop to run each pairwise iteration
# 
...
vsd <- varianceStabilizingTransformation(dds_lrt, blind=T)

#PCA Plots
library(ggplot2)
PCA=plotPCA(vsd, intgroup = c("phase2_cat","group"), ntop = 100000, returnData = TRUE)
percentVar <- round(100 * attr(PCA, "percentVar"))
PCA2 = ggplot(data = PCA, aes(PC1, PC2, color = phase2_cat,shape=group.1)) +
  geom_point(size=3) +
...
res1_lrt[order(res1_lrt["padj"]),]

volcano_lrt = EnhancedVolcano(res1_lrt,
                lab = rownames(res1_lrt),
                x = 'log2FoldChange',
                y = 'padj',
                pCutoff = 0.05,
                FCcutoff = 1.0,
                ...
                selectLab = rownames(res1)[res1$padj <= 0.0005], #label 20 most signifcant ASVs
                drawConnectors = TRUE, 
                title = "Most Significant Hits",
                subtitle = "",
                xlim = c(-5, 5))
ggsave("Output_Figures/Volcano_LRT.jpg", plot = volcano_lrt)
dev.off()
...
ColourStrategy = metadata$phase2_cat
...
heatmap.2(assay(vsd)[select,], col=viridis(256), Colv=TRUE,
          scale="row", key=T, keysize=1, symkey=T, labCol = ColourStrategy, ColSideColors=cols,lhei = c(1.5,5),
